...
if [ ${NUMERO_AMBIENTE} == "1" ] || [ ${NUMERO_AMBIENTE} == "2" ] || [ ${NUMERO_AMBIENTE} == "3" ] || [ ${NUMERO_AMBIENTE} == "4" ] || [ ${NUMERO_AMBIENTE} == "5" ] || [ ${NUMERO_AMBIENTE} == "6" ] || [ ${NUMERO_AMBIENTE} == "7" ] || [ ${NUMERO_AMBIENTE} == "8" ];
then
...
cp scripts/main-script-desa-front-desa1.sh ./${AMBIENTE_DESA}
...
echo;
echo "Copiando codigo fuente de FNETCORE a was80dea1 con usuario $1 (requiere password de acceso a was80desa1).";
echo "Ubicacion: ${PATH_TMP}/${AMBIENTE_DESA}";
pwd
tar czf - ${AMBIENTE_DESA} | ssh ${USUARIO}@was80desa1 bash -c "'
	tar xvzf - -C ${PATH_TMP} > /dev/null 2>&1
	cp ${PATH_TMP}/${AMBIENTE_DESA}/main-script-desa-front-desa1.sh  ${PATH_TMP}/${AMBIENTE_DESA}/main-script-desa-front-desa1_.sh
	chmod 777 ${PATH_TMP}/${AMBIENTE_DESA}/main-script-desa-front-desa1_.sh
	${PATH_TMP}/${AMBIENTE_DESA}/main-script-desa-front-desa1_.sh ${NUMERO_AMBIENTE}
'"

# y esto?
# sed 's/.$//' ${PATH_TMP}/${AMBIENTE_DESA}/main-script-desa-front-desa1.sh > ${PATH_TMP}/${AMBIENTE_DESA}/main-script-desa-front-desa1_.sh
